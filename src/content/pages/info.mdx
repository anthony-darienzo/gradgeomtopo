...
For `{{ getSiteConfig('semester') }}`, we are meeting on
*`{{getSiteConfig('meetingTime') }}`*, 
in **`{{ getSiteConfig('meetingLocation') }}`**.
...
A schedule of `{{ getSiteConfig('semester') }}` talks can be found [here](/upcoming).
